...
public sealed class BlackFastServerClient : BlackFastClient, IDisposable
{
    ...
    public BlackFastServerClient(UdpClient client, IPEndPoint remoteEndPoint,
        Guid sessionId, Action dispose) : base(client)
    {
        _dispose = dispose;
    ...
    internal Task StartAsync(CancellationToken cancellationToken)
        => Context.StartAsync(cancellationToken);
    ...
    internal Task ReadPackageAsync(ProtocolPackage package, CancellationToken cancellationToken)
        => Context.HandlePackageAsync(package, cancellationToken);
